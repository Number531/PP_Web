...
export function useAnimationState(completionThreshold = 0.95) {
  ...
  const handleScroll = useCallback(
    throttle(() => {
      ...
      setState((prev) => {
        ...
        if (isComplete !== prev.animationComplete) {
          ...
          if (isComplete) {
            ...
            // These timeouts will be handled outside setState to avoid stale closures
            return newState
          } else {
      ...
      const isComplete = currentExplosionProgress >= completionThreshold && progress >= 0.98
      if (isComplete && !state.showContent) {
    ...
    [state.animationComplete, state.overlayVisible, state.heroVisible, state.showContent, completionThreshold],
  )
